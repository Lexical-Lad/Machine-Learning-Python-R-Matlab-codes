...
#genderSVM<- svm(label~., training_set)
#genderRF <- randomForest(label~., training_set, importance =T)
#genderCART <- rpart(label~., training_set, method = 'class')
...
features <- specan3(data, parallel=1)

#features <- features[,4:24]

features <- features[, !names(features) %in% c('peakf'), drop = F]


f <- specan(data)


#features['Q25'] <- f['freq.Q25']
...
#prediction_SVM <- predict(genderSVM, features[,4:23])
#prediction_RF <- predict(genderRF, features[,4:23])
#prediction_CART <- predict(genderCART, features[,4:23])

#output_SVM <- data.frame("Audio File" = list, "Prediction/Label" =prediction_SVM)
#output_RF <- data.frame("Audio File" = list, "Prediction/Label" =prediction_RF)
#output_CART <- data.frame("Audio File" = list, "Prediction/Label" =prediction_CART)


prediction <- predict(model, features[,4:23])
